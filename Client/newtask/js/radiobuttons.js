...
style.type = 'text/css';
style.appendChild(document.createTextNode(`.radio::after {transition: opacity 0s ${timeout}s;}`));
...
function range(start, end, dirDown) {
    return [...Array(end - start).keys()].map(v => start + v + (dirDown ? 0 : 1));
}
